...
class FakeClinicalTablesSource:
    ...
    def search_dataset(self, dataset: str, request: object) -> SourceSearchResult:
        ...
        return self.responses_by_dataset[dataset]


class FakeCache:
    def __init__(self, entry: ProviderSearchCacheEntry | None = None) -> None:
        self.entry = entry
...
class ProviderSearchRankingTests(unittest.TestCase):
    ...
    def test_rank_provider_results_uses_cached_identity_as_stable_tiebreaker(self) -> None:
        ...
        self.assertEqual(ranked[0].provider.provider_id, cached_provider.provider_id)
        self.assertTrue(ranked[0].provider.ranking_metadata["cached_identity_match"])
        self.assertEqual(ranked[0].retriever_metadata["cache_hint"], "matched_prior_result")
